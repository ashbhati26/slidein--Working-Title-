...
export default function SignUpForm() {
  // Cast to any — Clerk's TS types lag behind the actual runtime API
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { signUp } = useSignUp() as any;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const { loaded: isLoaded, setActive, openSignUp } = useClerk() as any;
  ...
  redirectUrlComplete: "/dashboard",
      ...
      // Browser navigates away on success — nothing runs after this
    } catch (err: unknown) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if      (code === "form_identifier_exists")                                    setErrors({ email: "An account with this email already exists" });
      else if (code === "form_password_pwned" || code === "form_password_too_short") setErrors({ password: "Password is too weak. Choose a stronger one." });
      else setErrors({ root: e?.errors?.[0]?.message ?? "Something went wrong. Please try again." });
    } finally {
      setLoading(false);
    }
  }
    ...
    return (
      <div>
        <button
          ...
          style={{ display: "flex", alignItems: "center", gap: 5, fontSize: 12, color: "var(--ink-3)", background: "none", border: "none", cursor: "pointer", marginBottom: 16, padding: 0 }}
        >
        ...
        <form onSubmit={handleVerify} noValidate>
          ...
          <button
            ...
            style={{
              ...
              opacity: loading || !isLoaded ? 0.65 : 1,
            }}
          >
