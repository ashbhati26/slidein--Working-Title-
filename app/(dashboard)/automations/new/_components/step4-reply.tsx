...
interface Step4ReplyProps {
  ...
  onType: (t: ListenerType) => void;
  onMessage: (v: string) => void;
  onPublicReply: (v: string) => void;
...
export function Step4Reply({
  ...
  const canNext =
    listenerType === "fixed_message"
      ...
      : aiConfig.businessDescription.trim().length > 0 &&
        aiConfig.tone.trim().length > 0;

  function addFaq() {
    onAiConfig({ faqs: [...aiConfig.faqs, { question: "", answer: "" }] });
  }
  function updateFaq(i: number, f: "question" | "answer", v: string) {
    const u = [...aiConfig.faqs];
    u[i] = { ...u[i], [f]: v };
    onAiConfig({ faqs: u });
  }
  ...
  return (
    <div>
      ...
      >
        {isComment
          ? "What should Svation send when someone comments the keyword?"
          : "What should Svation reply when someone sends the keyword?"}
      </p>
      ...
      >
        {[
          {
            t: "fixed_message" as ListenerType,
            icon: MessageSquare,
            label: "Fixed reply",
            desc: "One pre-written message — brochure, price, link",
            locked: false,
          },
          {
            t: "smart_ai" as ListenerType,
            icon: Bot,
            label: "Smart AI",
            desc: "Full conversation in Hinglish, Hindi + 4 languages",
            locked: aiLocked,
          },
        ].map(({ t, icon: Icon, label, desc, locked }) => (
          <button
            key={t}
            onClick={() => !locked && onType(t)}
            style={{
              padding: "14px",
              borderRadius: 10,
              textAlign: "left",
          ...
          >
            ...
            <Icon
              size={16}
              color={listenerType === t ? "var(--accent)" : "var(--ink-3)"}
            ...
            <p
              style={{
                fontSize: 13,
                fontWeight: 500,
                color: listenerType === t ? "var(--accent)" : "var(--ink-1)",
                marginBottom: 2,
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          {isComment && (
            <div>
              ...
              <input
                value={publicReply}
                onChange={(e) => onPublicReply(e.target.value)}
              ...
              >
                Appears publicly under the comment — visible to everyone. The DM
                below is private.
              </p>
            </div>
          )}
          <div>
            <label style={labelSt}>
              {isComment ? "Private DM message" : "Reply message"}
            </label>
            <textarea
              value={message}
            ...
            <p style={{ fontSize: 11, color: "var(--ink-3)", marginTop: 4 }}>
              Tip: paste your Razorpay link, UPI ID, or PDF link directly in the
              message.
            </p>
          </div>
        </div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          {isComment && (
            <div>
              ...
              >
                Appears publicly. AI handles the private DM conversation.
              </p>
            </div>
          )}
          <div>
              ...
              {LANGS.map((lang) => (
                <button
                  ...
                  style={{
                    ...
                    borderRadius: 99,
                    fontSize: 12,
                    border: `1px solid ${aiConfig.language === lang.value ? "var(--accent)" : "var(--rule-md)"}`,
                    background:
                      aiConfig.language === lang.value
                        ? "var(--accent-muted)"
                        : "var(--bg)",
                    color:
                      aiConfig.language === lang.value
                        ...
                        : "var(--ink-2)",
                    cursor: "pointer",
                    fontWeight: aiConfig.language === lang.value ? 600 : 400,
                  }}
                >
                  {lang.label}
                </button>
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) =>
                onAiConfig({ businessDescription: e.target.value })
              }
              rows={3}
          ...
          <div>
            ...
            >
              <label style={{ ...labelSt, marginBottom: 0 }}>
                FAQs <span style={{ fontWeight: 400 }}>(optional)</span>
              </label>
            ...
            {aiConfig.faqs.length === 0 && (
              ...
              >
                Add common questions — the AI will answer them accurately.
              </p>
            )}
          </div>
          <div>
            <label style={labelSt}>
              Payment link <span style={{ fontWeight: 400 }}>(optional)</span>
            </label>
            <input
            ...
            />
            <p style={{ fontSize: 11, color: "var(--ink-3)", marginTop: 4 }}>
              AI shares this automatically when the buyer says they're ready.
            </p>
          </div>
          <div>
            <label style={labelSt}>
              Human escalation phrase{" "}
              <span style={{ fontWeight: 400 }}>(optional)</span>
            </label>
            <input
            ...
            />
            <p style={{ fontSize: 11, color: "var(--ink-3)", marginTop: 4 }}>
              When a lead sends this phrase, the AI stops and you get notified.
            </p>
          </div>
        </div>
      )}
