...
function Sk({ h = 80 }: { h?: number }) {
  return (
    <div style={{
      width: "100%", height: h, borderRadius: 14,
      background: "var(--rule)", animation: "sk 1.6s ease-in-out infinite",
    }} />
  );
}
...
export default function ReferralPage() {
  ...
  const referralLink = typeof window !== "undefined" && data?.referralCode
    ...
    : data?.referralCode
    ? `https://slidein.app/?ref=${data.referralCode}`
    : "";
    ...
    <div style={{ maxWidth: 680, margin: "0 auto", padding: "36px 28px 64px" }}>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <p style={{ fontSize: 13, color: "var(--ink-3)", letterSpacing: "-0.01em" }}>
          Share SlideIN and earn free months — or a 30% recurring commission.
        </p>
      </div>

      {loading && (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <Sk h={130} /><Sk h={96} /><Sk h={80} /><Sk h={220} /><Sk h={160} /><Sk h={180} />
